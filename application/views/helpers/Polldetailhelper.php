...
class Zend_View_Helper_Polldetailhelper{	
	...
	public function Polldetailhelper($row,$scoreDiv,$followstring,$followDiv,$ago)
	{	
		...
		$poloption = new Application_Model_Polloption();
			...
			//$db = '3685';
			$isPoll=1;
			//$pres=mysql_query('select * from tblPollVotes where PollID='.$db,$conn);
			...
			if($showChart) {
				$count=1;
				//$pres=mysql_query('select * from tblPollOptions where PollID='.$db.' ORDER BY ID ASC',$conn);
				$pres = $poloption->getpolloption($db);
				foreach($pres as $prow):
					//$total=mysql_num_rows(mysql_query("select * from tblPollVotes where Vote=".$prow->ID." AND PollID=".$db,$conn));
			...
			// GATHER USERS WHO TOOK PART IN THIS POLL
			$participants='';
			//----- TOTAL PARTICIPATING USERS ---------//
			//$TotalPartcpRes=mysql_query('select * from tblPollVotes,tblUsers where PollID='.$db.' AND tblPollVotes.User=tblUsers.UserID',$conn);
			
			$TotalPartcpRes = count($poloption->getpartcpres($db));
			
			//----- TOTAL PARTICIPATING USERS ---------//
			
			//$PartcpSQL='select * from tblPollVotes,tblUsers where PollID='.$db.' AND tblPollVotes.User=tblUsers.UserID limit 19';
			...
			if(count($PartcpRes)>0) {
				...
				$partStatusRow = $poloption->getpartstatusrow($db);	
					$partStatus=$partStatusRow['Status'];
					if($partStatus==1) $partLinkStart='<a href="profile.php?user='.$PartcpRow['UserID'].'">';
					else $partLinkStart='<a href="javascript:void(0)" class="profile-deactivated" title="'.DEACTIVE_ALT.'" onclick="return false;">';

					$participants.=$partLinkStart.'<div class="follower-box-profile"><img src="/show_thumbnails.php?ImgName='.$PartcpRow['ProfilePic'].'&ImgLoc=userpics&Width=35&Height=35" border="0" /></div></a>';
			...
			//$myvoteres=mysql_query('select * from tblPollVotes where PollID='.$db.' AND User='.$user,$conn);
			$user = '4';
			
			$myvoterow = $poloption->getmyvoteres($db,$user);			
			$myvoteres = count($myvoterow);
			if($myvoterow[0]['Vote']) {				
				$voteopdisplay='style="display:block"'; $myvotedisplay='style="display:block"';

				//$vsql = "select OptionText from tblPollOptions where ID=".$myvoterow->Vote;
				$vorteid = $myvoterow[0]['Vote'];
			...
			$dbee_highlighted='<div class="maindb-user-infobox"><div class="small-font-bold">'.$row['Name'].'</div>'.$profileLinkStart.'<div style="margin:10px 0px 10px 0px; width:130px; height:'.$imgH.'px; background-image:url(/show_thumbnails.php?ImgName='.$row['ProfilePic'].'&ImgLoc=userpics&Width=130&Height=130); background-repeat:no-repeat; background-position:center"></div></a>';
			if($showLinks)
			...
			endforeach;
			$dbee_highlighted.='</form><div class="maindb-addtofav" style="margin:20px 0 10px 0;"><a href="javascript:void(0)" rel="favourite-popup~300" class="poplight small-link" onclick="javascript:addtofavourite('.$row['DbeeID'].','.$row['User'].')" alt="add to favourites" title="add to favourites">add to favourites</a></div></div>';
			...
			$dbee_highlighted.='</div></div></div></div></div><div style="float:left; margin-left:25px;"><div class="medium-font-bold">Current Statistics</div><div id="PollPieChart" style="width:330px; height:230px;">';
			if(!$showChart) $dbee_highlighted.='No stats currently available';
			$dbee_highlighted.='</div></div></div></div><div>'.$followDiv.'<div id="poll-comment" style="float:left; margin-left:15px; display:none;"><textarea id="PollComment" class="roundedge-textbox fieldtext" style="width:280px; height:30px; margin-top:5px;" onKeyDown="limitText(\'PollComment\',200,\'pollcommcountdown\');" onKeyUp="limitText(\'PollComment\',200,\'pollcommcountdown\');" onfocus="if(this.value == \'write something to support your vote\') { this.value = \'\'; }" onblur="if(this.value == \'\') { this.value = \'write something to support your vote\'; }">write something to support your vote</textarea><br /><div id="pollcommcountdown" class="dbtextcountdown" style="margin:5px; color:#F9C10C; font-size:10px">200 limit</div></div><div id="maindb-scorewrapper" style="float:right; margin:5px 15px 0 0;"><div class="maindb-score-speecharrow"></div><div class="maindb-score">'.$scoreDiv.'<input type="hidden" id="hiddendb" value="'.$db.'"></div></div><div style="margin-left:160px">'.$participants.'</div></div><br style="clear:both; font-size:1px;">';
