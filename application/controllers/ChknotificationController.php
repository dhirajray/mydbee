...
class ChknotificationController extends Zend_Controller_Action
{
	  ...
	  public function indexAction()
	    {
	    	...
	    	if($checkFlag=='0') { // CHECK SINCE LAST LOGIN
	    		$CheckDateDbees=$dbeecoockes->currloginlastseen;
	    		$CheckDateMsgs=$dbeecoockes->currloginlastseen;
	    		$CheckDateGroups=$dbeecoockes->currloginlastseen;
	    		$CheckDateComments=$dbeecoockes->currloginlastseen;
	    		$CheckDateScore=$dbeecoockes->currloginlastseen;
	    	}
	    	elseif($checkFlag=='1') { // CHECK SINCE LAST SEEN IN CURRENT LOGIN
	    		$CheckDateDbees=$dbeecoockes->currloginlastseen;
	    		$CheckDateMsgs=$dbeecoockes->currloginlastseenmsg;
	    		$CheckDateGroups=$dbeecoockes->currloginlastseengroup;
	    		$CheckDateComments=$dbeecoockes->currloginlastseencomments;
	    		$CheckDateScore=$dbeecoockes->currloginlastscoretime;
	    	}
	    	...
	    	if($NotificationRow['Dbees']=='1') {
	    		if($users!='' && $CheckDateDbees!='') {
	    			$TDBRow= $DbeeallTable->getdbeenotification($users,$CheckDateDbees);
	    			$TotalDbees=$DbeeallTable->getdbeenotificationcnt($users,$CheckDateDbees);
	    	...
	    	if($CheckDateComments!='') {
	    		...
	    		if($dbs!='') {
	    			$seencomms=$dbeecoockes->seencomms;
	    			$TCdata = $NotificationRowdata->getscore($seencomms,$CheckDateComments,$dbs,$this->_userid);
	    			$count=1;
	    			$countTC=0;
	    			foreach($TCdata as $TCRow):
	    			$AllowedRow=$NotificationRowdata->getNotifyPop($TCRow['DbeeID'],$this->_userid);
	    			//$AllowedRow=@mysql_fetch_object($AllowedRes);
	    			...
	    			//		$TotalComments=mysql_num_rows($TCRes);
